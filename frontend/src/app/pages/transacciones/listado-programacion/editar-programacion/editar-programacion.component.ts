...
@Component({
  ...
  styleUrl: './editar-programacion.component.scss',
  providers: [
...
export class EditarProgramacionComponent {
  ...
  ngOnInit(): void {
    ...
    this.programacionService.getProgramacionById(this.programacionId).subscribe((res: any) => {
      ...
      this.programacionForm = new FormGroup({
        ...
        tipoServicio: new FormControl(res.tipoServicio, [Validators.required]),
        sectorTrabajo: new FormControl(res.sectorTrabajo, [Validators.required]),
        fecha: new FormControl(res.fecha, [Validators.required, this.fechaValidator]),
        vehiculo: new FormControl(res.vehiculo.id, [Validators.required]),
        observaciones: new FormControl(res.observaciones, [Validators.maxLength(500)])
      });
      this.getLocales();
    });
  ...
  onSubmit(): void {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.loading = true;
        this.programacionService.updateProgramacion(this.programacionId, this.programacionForm.value)
          .pipe(takeUntil(this.destroy$))
          .subscribe({
            next: (res: any) => {
